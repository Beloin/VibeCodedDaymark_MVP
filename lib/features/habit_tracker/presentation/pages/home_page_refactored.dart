...
class _HomePageRefactoredState extends State<HomePageRefactored> {
  ...
  HabitCalendarViewModel? _viewModel;
  ...
  void _showAddHabitModal() {
    AppLogger.i('Showing add habit modal', tag: 'HomePageRefactored');
  ...
  Widget _buildViewSwitcher(HabitCalendarViewModel viewModel) {
    return BlocBuilder<ConfigBloc, ConfigState>(
      builder: (context, configState) {
        ...
        AppLogger.i('_buildViewSwitcher rebuilding with viewType: $currentViewType', tag: 'HomePageRefactored');
        ...
        return SegmentedButton<ViewType>(
          ...
          onSelectionChanged: (Set<ViewType> newSelection) {
            final selectedView = newSelection.first;
            AppLogger.i('SegmentedButton selection changed to: $selectedView', tag: 'HomePageRefactored');
            viewModel.switchView(selectedView);
            
            // If switching to tile view, load historical data
            if (selectedView == ViewType.tile) {
              viewModel.loadHistoricalData();
            }

            if (selectedView == ViewType.calendar) {
            }
    ...
    return Scaffold(
        ...
        actions: [
          BlocBuilder<ConfigBloc, ConfigState>(
            builder: (context, configState) {
              ...
              return _buildViewSwitcher(viewModel);
            },
          ),
        ],
      ),
      ...
      body: MultiBlocListener(
        listeners: [
          ...
          // Removed problematic HabitBloc listener that was causing infinite loop
          // Historical data is now loaded only when switching to tile view or refreshing
        ],
        child: BlocBuilder<ConfigBloc, ConfigState>(
          builder: (context, configState) {
            AppLogger.i('Main content ConfigBloc rebuilding with state: ${configState.runtimeType}', tag: 'HomePageRefactored');
              ...
              builder: (context, state) {
                ...
                if (state is HabitLoading && !state.isRefreshing) {
                  ...
                  AppLogger.e(
                    ...
                    tag: 'HomePageRefactored', 
                    error: viewModel.errorMessage,
                    stackTrace: StackTrace.current,
                  );
                  content = DataLoadingErrorState(
                    onRetry: () {
                      AppLogger.i('Retrying data load after error', tag: 'HomePageRefactored');
                      context.read<HabitBloc>().add(const LoadHabits());
                    },
                  );
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: SizedBox(
                        ...
                        child: NoHabitsEmptyState(
                          onCreateHabit: () {
                            AppLogger.i('Create habit action triggered from empty state', tag: 'HomePageRefactored');
                            _showAddHabitModal();
                          },
                        ),
                      ),
                  ...
                  AppLogger.i('Building view content with currentViewType: ${viewModel.currentViewType}', tag: 'HomePageRefactored');
